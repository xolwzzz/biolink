...
router.get('/avatars/:filename', (req, res) => {
  const filePath = path.join(__dirname, '..', 'public', 'avatars', path.basename(req.params.filename));
  if (!fs.existsSync(filePath)) return res.status(404).end();
...
router.get('/api/profile/:username', (req, res) => {
  ...
  res.json({
    ...
    avatar_img_path: profile.avatar_img_path,
    links: profile.links,
    ...
    content_position: profile.content_position || 'bottom',
...
const RESERVED = ['login', 'signup', 'admin', 'auth', 'uploads', 'api', 'dashboard', 'avatars'];
