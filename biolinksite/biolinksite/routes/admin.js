...
router.post('/profile', requireAuth, (req, res) => {
  const { display_name, bio, font_family, links, enter_text } = req.body;
  ...
  const db = getDb();
  db.runSave(
    `UPDATE profile SET display_name=?, bio=?, font_family=?, links=?, enter_text=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?`,
    [
