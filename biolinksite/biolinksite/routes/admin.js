...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (['.mp4','.webm','.ogg','.mov'].includes(path.extname(file.originalname).toLowerCase())) return cb(null, true);
    cb(new Error('Only video files are allowed.'));
  },
});

router.get('/dashboard', requireAuth, (req, res) => res.sendFile(path.join(__dirname, '..', 'public', 'dashboard.html')));
...
router.get('/profile', requireAuth, (req, res) => {
  ...
  profile.video_muted = profile.video_muted === 1;
  res.json({ profile, username: req.session.username });
});

router.post('/profile', requireAuth, (req, res) => {
  const { display_name, bio, font_family, links, video_muted } = req.body;
  ...
  const db = getDb();
  db.runSave(
    `UPDATE profile SET display_name=?, bio=?, font_family=?, links=?, video_muted=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?`,
    [(display_name||'').slice(0,60), (bio||'').slice(0,300), safe_font, linksStr, video_muted ? 1 : 0, req.session.userId]
  );
